...
@section('title', 'Edit ' . $paidAmenity->name)

@php
    $title = 'Edit Service';
    $subtitle = 'Update service details and settings';
    $showBackButton = true;
    $backUrl = route('paid-amenities.show', $paidAmenity);
@endphp
...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6" style="background-color: var(--card-bg); border-color: var(--border-color);">
        <form method="POST" action="{{ route('paid-amenities.update', $paidAmenity) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" required
                               value="{{ old('name', $paidAmenity->name) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400"
                                  placeholder="Detailed description of the service...">{{ old('description', $paidAmenity->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category" id="category" required
                            ...
                            <option value="food" {{ old('category', $paidAmenity->category) == 'food' ? 'selected' : '' }}>Food & Meals</option>
                            <option value="cleaning" {{ old('category', $paidAmenity->category) == 'cleaning' ? 'selected' : '' }}>Cleaning Services</option>
                            <option value="laundry" {{ old('category', $paidAmenity->category) == 'laundry' ? 'selected' : '' }}>Laundry Services</option>
                            <option value="utilities" {{ old('category', $paidAmenity->category) == 'utilities' ? 'selected' : '' }}>Utilities</option>
                            <option value="services" {{ old('category', $paidAmenity->category) == 'services' ? 'selected' : '' }}>General Services</option>
                            <option value="other" {{ old('category', $paidAmenity->category) == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                        @error('category')
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="billing_type" id="billing_type" required onchange="updateBillingInfo()"
                                ...
                                <option value="monthly" {{ old('billing_type', $paidAmenity->billing_type) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                                <option value="daily" {{ old('billing_type', $paidAmenity->billing_type) == 'daily' ? 'selected' : '' }}>Daily (Pay per use)</option>
                            </select>
                            @error('billing_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" id="price" step="0.01" min="0" required
                                       value="{{ old('price', $paidAmenity->price) }}"
                    ...
                    <div id="dailyOptions" class="{{ $paidAmenity->billing_type === 'daily' ? '' : 'hidden' }}">
                        ...
                        <input type="number" name="max_usage_per_day" id="max_usage_per_day" min="1"
                               value="{{ old('max_usage_per_day', $paidAmenity->max_usage_per_day) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               style="background-color: var(--input-bg); border-color: var(--border-color); color: var(--text-primary);"
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                            @php
                                ...
                                $selectedDays = old('availability_days', $paidAmenity->availability_schedule['days'] ?? []);
                            @endphp
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" id="icon"
                               value="{{ old('icon', $paidAmenity->icon) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                               style="background-color: var(--input-bg); border-color: var(--border-color); color: var(--text-primary);"
                    ...
                    <div>
                        ...
                        <textarea name="terms_conditions" id="terms_conditions" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                  style="background-color: var(--input-bg); border-color: var(--border-color); color: var(--text-primary);"
                                  placeholder="Service terms, conditions, and important notes...">{{ old('terms_conditions', $paidAmenity->terms_conditions) }}</textarea>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                               {{ old('is_active', $paidAmenity->is_active) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
                ...
                <!-- Warning for existing assignments -->
                @if($paidAmenity->tenantAmenities->count() > 0)
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        <div class="flex items-start">
                            <i class="fas fa-exclamation-triangle text-yellow-600 mt-1 mr-3"></i>
                            <div>
                                <h4 class="text-sm font-medium text-yellow-800">Service Currently Assigned</h4>
                                <p class="text-sm text-yellow-700 mt-1">
                                    This service is currently assigned to {{ $paidAmenity->tenantAmenities->count() }} tenant(s).
                                    Changes to pricing and billing type may affect existing assignments.
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex gap-4 pt-4">
                    <a href="{{ route('paid-amenities.show', $paidAmenity) }}"
