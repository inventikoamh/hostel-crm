...
@php
    ...
    $showBackButton = true;
    $backUrl = route('config.amenities.show', $amenity->id);
@endphp

@section('content')
<div class="max-w-2xl mx-auto">
    <form action="{{ route('config.amenities.update', $amenity->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6" style="background-color: var(--card-bg); border-color: var(--border-color);">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="icon" name="icon" value="{{ old('icon', $amenity->icon) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           style="background-color: var(--bg-secondary); border-color: var(--border-color); color: var(--text-primary);"
                ...
                <div>
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $amenity->sort_order) }}" min="0"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                           style="background-color: var(--bg-secondary); border-color: var(--border-color); color: var(--text-primary);"
                           placeholder="0">
                    @error('sort_order')
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              style="background-color: var(--bg-secondary); border-color: var(--border-color); color: var(--text-primary);"
                              placeholder="Enter amenity description (optional)">{{ old('description', $amenity->description) }}</textarea>
                    @error('description')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="flex items-center justify-end space-x-4">
            <a href="{{ route('config.amenities.show', $amenity->id) }}"
